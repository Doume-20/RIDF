...
class Version {
    static Parse(text) {
        ...
        const result = { main: [], negative: [], positive: [] };

        let [start, current] = [-1, result.main];
        for (let i = 0; i < parts.length; i++) {
            const separator = start === -1 ? '.' : text[start];
            if (separator === '+') {
    ...
    static CompareNumber(x, y) {
        if (x < y) return -1;
        if (x > y) return 1;
        return 0;
    }

    static ComparePart(x, y) {
        const xn = Number(x);
        const yn = Number(y);

        if (!isNaN(xn) && !isNaN(yn))
            return Version.CompareNumber(xn, yn);

        if (!isNaN(xn))
            return -1; // Number < Identifier

        if (!isNaN(yn))
            return 1; // Identifier > Number
    ...
    static CompareParts(x, y) {
        const lx = x.length;
        const ly = y.length;
        const l = Math.min(lx, ly);
        for (let i = 0; i < l; i++) {
            const r = Version.ComparePart(x[i], y[i]);
            if (r !== 0) return r;
        }
        return Version.CompareNumber(lx, ly);
    }

    static Compare(x, y) {
        const vx = Version.Parse(x);
        const vy = Version.Parse(y);

        let result = Version.CompareParts(vx.main, vy.main);
        if (result !== 0)
            return result;

        if (vx.negative.length !== 0 || vy.negative.length !== 0) {
            if (vx.negative.length === 0)
                return 1; // No negative is greater than any negative

            if (vy.negative.length === 0)
                return -1; // All negatives are less than no negative

            result = Version.CompareParts(vx.negative, vy.negative);
            if (result !== 0)
                return result;
        }

        return Version.CompareParts(vx.positive, vy.positive);
    }
}
...
class SessionExecutor {
    constructor(executor, system = false, hidden = true, service = null) {
    ...
    get minimum() {
        // The "variables" parameter is not supported before this version.
        return "1.0.4";
    }

    async verify() {
        const result = await this.service.executeScript(`$v = (Get-Item '${this.host}' -ErrorAction SilentlyContinue).VersionInfo.ProductVersion ; if ($v) { $v } else { "0.0.0" }`, [], "Sqool-Net check");

        if (Version.Compare(result.output, this.minimum) < 0)
            throw "Required version of Sqool-Net is not available";
    }
    ...
    async execute(command, args = [], env = {}) {
        ...
        return await this.executor.execute(this.host, ['asuser', '--modern', '--wait', ...system, ...hidden, '--working-dir', directory(), '--variables', JSON.stringify(Object.keys(env)), command, ...args], env);
    }
}
...
class Execution {
    ...
    interactive(system = false, hidden = true) {
        const command = new CommandEngine(new SessionExecutor(new DefaultExecutor(), system, hidden, this.service()));
